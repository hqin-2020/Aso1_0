...
def minimization(start):
    ...
    def init_kf(θ):
        ...
        λ, η, b11, b22, As11, As12, As13, As21, As22, As23, As24, As31, As32, As33, As34, Bs11, Bs21, Bs22, Bs31, Bs32, Bs33 = θ
        ones = np.ones([3,1])
    ...
    def ll(θ):
        ...
        B = np.array([[b11, 0,   0,     0,     0],\
                      ...
                      [0,   0,   Bs31,  Bs32,  Bs33],\
                      [0,   0,   0,       0,   0]])
        D = np.array([[1,   1,   As11,  As12,  As13,  0],\
    ...
    if np.all(abs(np.linalg.eigvals(Ass))<1):
        try:
            θ_opt = sp.optimize.minimize(ll, start, method = 'L-BFGS-B', bounds = bnds, callback = callback, tol=1e-7)  
